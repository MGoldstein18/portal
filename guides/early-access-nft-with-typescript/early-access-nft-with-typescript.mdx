...
Our first step is to create a bundle drop module. This can be done in the [dashboard](/dashboard) and if you aren’t sure how to create the module please check [this guide.](/portal/learn/introduction) 
...
Now that we have our NFT, we need to set the start time, price, and supply. We can also set the number of NFTs that can be claimed in one transaction, create a white-list and adjust other settings. You can see a [full example here](/portal/learn/code-examples/bundle-drop#setting-claim-conditions) (look at the 3rd example).
...
Before users can claim an NFT, they need to connect their wallets. Thirdweb makes this incredibly easy! We can simply use the `ThirdwebProvider` context with the `ConnectWallet` component to make it happen. For all the details, please check out [this guide](/portal/guides/add-connectwallet-to-your-website). 
...
**If you want to take this project further, you can explore how we could use the `balanceOf` method to check whether or not a user has minted one of our NFTs! Check out our code examples [here](/portal/learn/code-examples/bundle-drop).**